...
@give_group.command(name="hp", description="Set HP (0 = full)")
@owner_only_check(OWNER_ID)
async def give_hp(inter: discord.Interaction, nguoi_dung: discord.Member,
                  hp: app_commands.Range[int, 0, 9999999]):
    await inter.response.defer(ephemeral=False)
    ts = await get_tu_si(nguoi_dung.id)
    if not ts: return await safe_followup(inter, embed=e_loi("Lỗi", f"**{nguoi_dung.display_name}** chưa tu tiên!"))
    new_hp = ts["hp_max"] if hp == 0 else hp
    await update_tu_si(nguoi_dung.id, hp=new_hp)
    await safe_followup(inter, embed=_give_embed(nguoi_dung, "Sinh Lực", f"❤️ **{new_hp}**"))
...
@give_group.command(name="all", description="Max out toàn bộ nhân vật")
@owner_only_check(OWNER_ID)
async def give_all(inter: discord.Interaction, nguoi_dung: discord.Member):
    await inter.response.defer(ephemeral=False)
    ts = await get_tu_si(nguoi_dung.id)
    if not ts: return await safe_followup(inter, embed=e_loi("Lỗi", "Chưa tu tiên!"))
    all_pb = list(range(len(PHAP_BAO)))
    all_dd = {str(i): 99 for i in range(len(DAN_DUOC))}
    await update_tu_si(nguoi_dung.id,
        canh_gioi=9, cap_nho=3, linh_can=0,
        linh_thach=9999999, hp=999999, hp_max=999999, cong=99999, thu=99999,
        phap_bao=all_pb, dan_duoc=all_dd, exp=0)
    embed = discord.Embed(title="👑 MAX OUT!", color=0xFFD700,
        description=f"**{ts['dao_hieu']}** đã được nâng lên đỉnh cao!")
    embed.add_field(name=f"{E_TU_VI} Tu Vi",      value="👑 Tam Kỳ Đăng Tiên",      inline=True)
    embed.add_field(name="🌟 Thiên Phú",  value="🌟 Thiên Linh Căn ×4.0", inline=True)
    embed.add_field(name=f"{E_LINH_THACH} Linh Thạch", value="9,999,999",               inline=True)
    embed.set_thumbnail(url=nguoi_dung.display_avatar.url)
...
@give_group.command(name="dotpha", description="Test đột phá — force thành công hoặc thất bại")
@app_commands.describe(nguoi_dung="Người test", ket_qua="thanh_cong / that_bai")
@app_commands.choices(ket_qua=[
    app_commands.Choice(name="✅ Thành công", value="thanh_cong"),
    app_commands.Choice(name="❌ Thất bại",   value="that_bai"),
])
@owner_only_check(OWNER_ID)
async def give_dotpha(inter: discord.Interaction, nguoi_dung: discord.Member,
                      ket_qua: str = "thanh_cong"):
    await inter.response.defer(ephemeral=True)
    ts = await get_tu_si(nguoi_dung.id)
    if not ts:
    ...
    if ket_qua == "thanh_cong":
        ...
        new_cg   = ts["canh_gioi"]
        la_dai   = (ts["cap_nho"] >= max_cap)
        if new_cap > max_cap:
            new_cap = 1
            new_cg  = min(new_cg + 1, len(CANH_GIOI) - 1)
        new_hp   = hp_max_cong_thuc(new_cg, new_cap)
        new_at   = cong_cong_thuc(new_cg, new_cap)
        new_df   = thu_cong_thuc(new_cg, new_cap)
        tong     = ts.get("tong_tu_vi", 0) + ts["exp"]
        new_exp  = 0 if new_cg > ts["canh_gioi"] else ts["exp"]
        linh_luc = int((200 + new_cg**2 * 2300 + new_cap * 230) * 0.8 * 0.7)

        lv_g   = new_cg * 9 + new_cap
        ht_g   = int(new_at * 0.08 + lv_g * 3)
        ho_g   = int(new_df * 0.15 + lv_g * 2)
        bk_g   = min(5 + new_cg * 3 + new_cap, 75)
        kb_g   = min(3 + new_cg * 2 + new_cap // 2, 50)
        if la_dai:
            # Đại cảnh — update toàn bộ chỉ số
            await update_tu_si(nguoi_dung.id,
                canh_gioi=new_cg, cap_nho=new_cap, exp=new_exp,
                hp=new_hp, hp_max=new_hp, cong=new_at, thu=new_df,
                linh_luc=linh_luc, hoi_tam=ht_g, ho_tam=ho_g,
                bao_kich=bk_g, khang_bao=kb_g,
                tong_tu_vi=tong if new_cg > ts["canh_gioi"] else ts.get("tong_tu_vi", 0))
        else:
            # Tiểu cảnh — chỉ update HP và Linh Lực
            await update_tu_si(nguoi_dung.id,
        ...
        ts_new   = await get_tu_si(nguoi_dung.id)
        dao_hieu = ts_new.get("dao_hieu", nguoi_dung.display_name)
        cg_obj   = get_cg(new_cg)
        lv       = new_cg * 9 + new_cap

        if la_dai:
            hoi_tam   = int(new_at * 0.08 + lv * 3)
            ho_tam    = int(new_df * 0.15 + lv * 2)
            bao_kich  = min(5 + new_cg * 3 + new_cap, 75)
            khang_bao = min(3 + new_cg * 2 + new_cap // 2, 50)
            embed = e_ok("🌟 [TEST] ĐỘT PHÁ THÀNH CÔNG!", f"**{dao_hieu}** đã phá vỡ rào cản, tiến nhập vào cảnh giới **{cg_obj['ten']}**!")
            embed.set_author(name=dao_hieu, icon_url=nguoi_dung.display_avatar.url)
            embed.add_field(name="Cảnh giới mới",
                value=f"{cg_obj['emoji']} **{get_cg_ten(new_cg, new_cap)}**", inline=False)
            _e = get_stat_emoji
            ...
            embed.add_field(name=f"{_e('phong_ngu')} Phòng Ngự",value=fmt(new_df),      inline=True)
            ...
            embed.set_footer(text="[TEST] Căn cơ đã ổn định!")
            gif = "images/thanhcong.gif"
            if os.path.exists(gif):
                await safe_followup(inter, embed=embed, file=discord.File(gif, "thanhcong.gif"), ephemeral=True)
            else:
                await safe_followup(inter, embed=embed, ephemeral=True)
        else:
            embed = e_ok("✨ [TEST] THĂNG TIẾN THÀNH CÔNG!", f"**{dao_hieu}** đã bước vào **{cg_obj['emoji']} {get_cg_ten(new_cg, new_cap)}**!")
            embed.add_field(name=f"{get_stat_emoji('sinh_luc')} Sinh Lực", value=fmt(new_hp),   inline=True)
            embed.add_field(name=f"{get_stat_emoji('linh_luc')} Linh Lực",  value=fmt(linh_luc), inline=True)
            await safe_followup(inter, embed=embed, ephemeral=True)
    else:
        # Thất bại — chỉ áp dụng cho đại cảnh
        if ts["cap_nho"] < max_cap:
            return await safe_followup(inter, 
                embed=e_loi("Không hợp lệ", "Thất bại chỉ xảy ra ở **Đại Cảnh** (Hậu Kì → CG mới)."), ephemeral=True)
        tran     = max(0, ts["exp"] - ec)
        exp_sot  = ts["exp"] - tran
        exp_mat  = int(exp_sot * 0.15)
        exp_con  = exp_sot - exp_mat
        import time as _t
        await update_tu_si(nguoi_dung.id, exp=exp_con, cd_dot_pha=int(_t.time()))
        thua_str = f"-{fmt(tran + exp_mat)} {E_TU_VI}"
        if tran > 0:
            thua_str += f"\n*(Tràn: -{fmt(tran)} + 15%: -{fmt(exp_mat)})*"
        dao_hieu_f = ts.get("dao_hieu", nguoi_dung.display_name)
        embed = discord.Embed(title="💀 [TEST] ĐỘT PHÁ THẤT BẠI", color=0xED4245,
            description="Tâm ma trỗi dậy, linh lực nghịch chuyển. Đạo hữu đã đột phá thất bại!")
        embed.set_author(name=dao_hieu_f, icon_url=nguoi_dung.display_avatar.url)
        ...
        embed.set_footer(text="[TEST] Kiên trì luyện tập, thử lại sau!")
        gif = "images/thatbai.gif"
        if os.path.exists(gif):
            await safe_followup(inter, embed=embed, file=discord.File(gif, "thatbai.gif"), ephemeral=True)
        else:
            await safe_followup(inter, embed=embed, ephemeral=True)
    await safe_followup(inter, f"✅ Test xong cho **{nguoi_dung.display_name}**", ephemeral=True)


@give_group.command(name="thechat", description="[Owner] Đổi thể chất cho người chơi")
@app_commands.describe(
    nguoi_dung="Người chơi cần đổi thể chất",
    the_chat_id="ID thể chất (để trống để xem danh sách)"
)
@owner_only_check(OWNER_ID)
async def give_the_chat(inter: discord.Interaction, nguoi_dung: discord.Member, the_chat_id: str = ""):
    await inter.response.defer(ephemeral=True)
    ...
    await safe_followup(inter, 
        f"✅ Đã đổi thể chất **{nguoi_dung.display_name}**:\n"
...
@give_group.command(name="quadenbu", description="[Owner] Gửi quà cảm ơn cho tất cả người chơi")
@owner_only_check(OWNER_ID)
async def give_qua_den_bu(inter: discord.Interaction):
    await inter.response.defer(ephemeral=True)
    from utils.config import LINH_QUA, MANH_LINH_CAN_EMOJI, DOTPHA_TC_NGUYEN_LIEU, LINH_QUA_BY_ID
    from utils.database import get_bang_xep_hang
    import json as _json

    # Lấy tất cả user_id từ DB (lấy top 9999)
    all_users = await get_bang_xep_hang(top=9999)
    if not all_users:
        return await safe_followup(inter, "❌ Không có người chơi nào!", ephemeral=True)

    await safe_followup(inter,
        f"⏳ Đang gửi quà cảm ơn cho **{len(all_users)}** người chơi...", ephemeral=True)

    # Quà cảm ơn
    LINH_THACH_BU       = 100_000
    LINH_QUA_SO_LUONG   = 1   # mỗi loại
    MANH_SO_LUONG       = 1   # mỗi loại
    # DTC x1 mỗi loại

    from utils.config import MANH_LINH_CAN_EMOJI as _MANH_EMO, LINH_CAN_BY_ID

    # Build embed DM
    dm_embed = discord.Embed(
        title="🎁 CẢM ƠN ĐÃ ĐỒNG HÀNH CÙNG THIÊN ĐẠO!",
        description=(
            "Kính gửi các đạo hữu,\n\n"
            "Cảm ơn mọi người đã tham gia thời gian qua! 💛\n\n"
            "**Thiên Đạo** xin gửi tặng phần quà nhỏ để tri ân:"
        ),
        color=0xFFD700
    )
    dm_embed.add_field(
        name=f"{E_LINH_THACH} Linh Thạch",
        value=f"**+{fmt(LINH_THACH_BU)}**",
        inline=True
    )
    lq_lines = " · ".join(
        f"{LINH_QUA_BY_ID[lq['id']]['emoji']}×{LINH_QUA_SO_LUONG}"
        for lq in LINH_QUA if lq['id'] in LINH_QUA_BY_ID
    )
    dm_embed.add_field(
        name="🌿 Linh Quả (mỗi loại)",
        value=lq_lines,
        inline=False
    )
    manh_lines = " · ".join(
        f"{emo}×{MANH_SO_LUONG}"
        for emo in _MANH_EMO.values()
    )
    dm_embed.add_field(
        name="🔶 Mảnh Linh Căn (mỗi loại)",
        value=manh_lines,
        inline=False
    )
    dtc_lines = " · ".join(
    ...
    dm_embed.add_field(
        name="🧬 Tài Nguyên Đột Phá TC",
        value=dtc_lines,
        inline=False
    )
    dm_embed.set_footer(text="Thiên Đạo — Cảm ơn và hẹn gặp lại! 🌙")
    ...
    for user_row in all_users:
        ...
        try:
            ...
            # Cộng linh thạch
            new_lt = ts["linh_thach"] + LINH_THACH_BU

            # Cộng linh quả
            lq_new = ts.get("linh_qua", {}).copy()
            for lq in LINH_QUA:
                lq_new[lq["id"]] = lq_new.get(lq["id"], 0) + LINH_QUA_SO_LUONG

            # Cộng mảnh linh căn
            manh_new = ts.get("manh_linh_can", {}).copy()
            for k in _MANH_EMO.keys():
                manh_new[k] = manh_new.get(k, 0) + MANH_SO_LUONG

            # Cộng tài nguyên ĐP TC
            raw_dtc = ts.get("dotpha_tc_nl", {})
            if isinstance(raw_dtc, str):
                try: dtc_new = _json.loads(raw_dtc)
                except: dtc_new = {}
            else:
                dtc_new = raw_dtc.copy()
            for nl in DOTPHA_TC_NGUYEN_LIEU:
                dtc_new[nl["id"]] = dtc_new.get(nl["id"], 0) + 1

            await update_tu_si(uid,
                linh_thach=new_lt,
                linh_qua=lq_new,
                manh_linh_can=manh_new,
                dotpha_tc_nl=dtc_new
            )

            # Gửi DM
            try:
                user_obj = inter.client.get_user(uid) or await inter.client.fetch_user(uid)
                if user_obj:
                    await user_obj.send(embed=dm_embed)
            except discord.HTTPException:
                pass  # DM thất bại (user tắt DM) không ảnh hưởng
    ...
    await safe_followup(inter,
        f"✅ Đã gửi quà cảm ơn cho **{ok_count}** người chơi.\n"
        f"{'⚠️ ' + str(fail_count) + ' người bị lỗi.' if fail_count else ''}",
        ephemeral=True
    )
...
@give_group.command(name="removethechat", description="[Owner] Xóa thể chất của người chơi (profile hiển thị chưa có)")
@app_commands.describe(nguoi_dung="Người chơi cần xóa thể chất")
@owner_only_check(OWNER_ID)
async def give_remove_the_chat(inter: discord.Interaction, nguoi_dung: discord.Member):
    await inter.response.defer(ephemeral=True)
    ts = await get_tu_si(nguoi_dung.id)
    if not ts:
        return await safe_followup(inter, "❌ Người chơi chưa có hồ sơ!", ephemeral=True)
    ...
    embed = _give_embed(nguoi_dung, "Xóa Thể Chất",
        f"{old_name} → *(chưa có)*")
    await safe_followup(inter, embed=embed, ephemeral=True)
